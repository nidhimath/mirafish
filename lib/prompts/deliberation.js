...
export function buildDeliberationPrompt(caseDoc, reactions, jurors) {
  const jurorProfiles = jurors.map((j) => {
    ...
    return `[id: ${j.id}] ${j.name} (${j.demographics?.occupation}, ${j.demographics?.age}yo ${j.demographics?.gender}) — initial lean: ${lean} / ${confidence} confidence
  ...
  const idRoster = jurors.map((j) => `  - ${j.id} = ${j.name}`).join('\n');
...
CRITICAL — speaker_id format:
- Every transcript turn MUST set speaker_id to one of the exact juror IDs from the roster (e.g. "juror_001"), NOT a name or nickname.
- Every speaker_name MUST be the juror's first name as listed in the roster.
- Do NOT invent new speakers. Every turn must come from a juror in the roster.`;
...
--- JUROR ID ROSTER (use these EXACT ids in speaker_id) ---
...
REMINDER: speaker_id must be one of the exact ids from the roster above (e.g. "juror_001"). Do not use names, slugs, or invented ids.`;
